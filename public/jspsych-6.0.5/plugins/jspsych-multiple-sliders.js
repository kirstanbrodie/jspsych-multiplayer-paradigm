/**
 * jspsych-multiple-sliders
 *
 */


jsPsych.plugins['multiple-sliders'] = (function() {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      button_label: {
          ...
          default:  'Continue',
          array: false,
          description: 'Label of the button to advance.' //currently disabled
      },
      stimulus_duration: {
          type: jsPsych.plugins.parameterType.INT,
  ...
  plugin.trial = function(display_element, trial) {
    ...
    let html = '<div id="jspsych-canvas-sliders-response-wrapper" class = "jspsych-canvas-sliders-response-wrapper">';
    html += '<div id="jspsych-canvas-sliders-response-stimulus">' + trial.stimulus + '</div>';
    // Prompt text
    ...
    // add submit button
    html += button = '<button id="jspsych-canvas-sliders-response-next" class="jspsych-btn">'+trial.button_label+'</button>';
    ...
    display_element.querySelector('#jspsych-canvas-sliders-response-next').addEventListener('click', function() {
        ...
        for (let i=0; i<sliders.length; i++) {
            ...
            answers = {...answers,
                //id: i,
                //name: slider.name,
                ...{ [slider.name + "_answer"]: slider.value,
                  [slider.name + "_prompt"]: slider.prompt,
                  [slider.name + "_lastChangedTime"]: slider.changedTime
                }
            }
        ...
        if(trial.response_ends_trial){
            ...
            display_element.querySelector('#jspsych-canvas-sliders-response-next').disabled = false;
        }
